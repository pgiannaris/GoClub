...
export function AppBreadcrumbs(props: {
  ...
  const normalizedPath = splitPath.filter((segment, index) => {
    return !(segment === 'projects' && splitPath[index - 1] === 'home');
  });
  ...
  const showEllipsis = normalizedPath.length > maxDepth;

  const visiblePaths = showEllipsis
    ? ([normalizedPath[0], ...normalizedPath.slice(-maxDepth + 1)] as string[])
    : normalizedPath;
