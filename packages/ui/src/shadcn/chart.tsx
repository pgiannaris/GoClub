...
> = ({ id, className, children, config, responsiveProps, ...props }) => {
  const uniqueId = React.useId();
  const chartId = `chart-${id ?? uniqueId.replace(/:/g, '')}`;
  const mergedResponsiveProps = React.useMemo(
    () => ({
      debounce: 140,
      minWidth: 0,
      ...responsiveProps,
  ...
  return (
    <ChartContext.Provider value={{ config }}>
      ...
      >
        ...
        <RechartsPrimitive.ResponsiveContainer {...mergedResponsiveProps}>
          {children}
