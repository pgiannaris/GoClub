...
import { resolveAllowedAuthRedirectUrl } from './cookie-options';
...
class AuthCallbackService {
  ...
  async verifyTokenHash(
    ...
    const callbackUrl = resolveAllowedAuthRedirectUrl(callbackParam, url);
    ...
    if (callbackUrl) {
      ...
      if (callbackNextPath) {
        nextUrl =
          resolveAllowedAuthRedirectUrl(callbackNextPath, callbackUrl) ??
          new URL(params.redirectPath, callbackUrl.origin);
      } else {
        nextUrl = new URL(
  ...
  async exchangeCodeForSession(
    ...
    const nextUrl = resolveAllowedAuthRedirectUrl(
    ...
    return {
      nextPath: nextUrl?.toString() ?? params.redirectPath,
    };
  }
}
