...
export default function MembersPage() {
    ...
    const fetchMembers = async () => {
      ...
      const [
        ...
        (supabase as any)
          ...
          .select(
            'id, project_id, account_id, role, invited_by, joined_at, created_at',
          )
      ...
      try {
        const response = await fetch(
          `/api/projects/${encodeURIComponent(projectId)}/members/accounts`,
          {
            credentials: 'include',
          },
        );
  ...
  const canManageRoster = useMemo(() => {
    ...
    const ownMembership = projectMembers.find(
      (member) => member.account_id === currentAccountId,
    );
  ...
  const websiteAccountIds = useMemo(
    () => new Set(websiteAccounts.map((a) => a.id)),
    [websiteAccounts],
  );

  const ownerMember = useMemo(() => {
    if (ownerId) {
      const byOwnerId = projectMembers.find(
        (member) => member.account_id === ownerId,
      );
  ...
  const collaborators = useMemo(() => {
    return projectMembers
      .filter(
        (member) =>
          !ownerMember || member.account_id !== ownerMember.account_id,
      )
      .sort(
        (a, b) =>
  ...
  const inviteMember = async () => {
    ...
    try {
      const response = await fetch(
        `/api/projects/${encodeURIComponent(projectId)}/members/invite`,
        {
          method: 'POST',
          credentials: 'include',
          headers: {
  ...
  const addWebsiteAccountToRoster = async (account: WebsiteAccount) => {
    ...
    try {
      ...
      if (!response.ok || !payload.student) {
        throw new Error(
          payload.error || 'Failed to add account to the student roster',
        );
      }
      ...
      setSiteUsers((prev) =>
        prev.filter((item) => item.accountId !== account.id),
      );
      ...
      if (payload.alreadyExists) {
        ...
        toast.success(
          `${account.name} linked to an existing student roster entry`,
        );
      } else {
      ...
      const message =
        error instanceof Error
          ? error.message
          : 'Failed to add account to the student roster';
      toast.error(message);
    } finally {
    ...
    <div className="space-y-6">
      <div className="flex flex-wrap items-start justify-between gap-3">
        <div>
          ...
          <p className="text-muted-foreground">
            Students, owner, and collaborators with access to attendance and
            GoClub features.
          </p>
        </div>
        {canManageRoster && (
        ...
        <div className="text-muted-foreground w-full text-xs md:text-right">
      ...
      <div className="grid gap-6 xl:grid-cols-2">
          ...
          <CardHeader>
            <div className="flex items-start justify-between gap-3">
              <div>
                ...
                <CardDescription>
                  Public roster used by attendance and club-facing pages (
                  {studentsSorted.length} total).
                </CardDescription>
                {canManageRoster && (
                  <p className="text-muted-foreground mt-1 text-xs">
                    Click a student to rename them or remove them from the
                    class.
                  </p>
                )}
              </div>
          ...
          <CardContent className="space-y-3">
            ...
            {!loading && studentsSorted.length === 0 && (
              <div className="text-muted-foreground rounded-md border border-dashed p-4 text-sm">
                No students yet. Use the add student button to move website
                accounts into the attendance roster.
              </div>
            )}
            ...
            {!loading &&
              studentsSorted.map((student) => (
                <button
                  key={student.id}
                  type="button"
                  className="hover:bg-muted/30 w-full rounded-md border p-4 text-left transition-colors"
        ...
        <Card>
          <CardHeader>
            ...
            <CardDescription>
              Project-level access users who can view or manage attendance and
              club settings ({projectMembers.length} total).
            </CardDescription>
          </CardHeader>
          <CardContent className="space-y-4">
            <div className="space-y-2">
              ...
              {loading && (
                <div className="bg-muted/30 h-16 animate-pulse rounded-md border" />
              )}
              {!loading && !ownerMember && (
                <div className="text-muted-foreground rounded-md border border-dashed p-4 text-sm">
              ...
              {loading &&
                Array.from({ length: 2 }).map((_, index) => (
                  <div
                    ...
                    className="bg-muted/30 h-16 animate-pulse rounded-md border"
                  />
                ))}
            ...
            <div className="space-y-2">
              ...
              {!loading && collaborators.length === 0 && (
                <div className="text-muted-foreground rounded-md border border-dashed p-4 text-sm">
                  No collaborators yet. Invite admins, members, or viewers to
      ...
      <Card>
        <CardHeader>
          ...
          <CardDescription>
            Signed-in users who selected a public-site role for this club (
            {siteUsersSorted.length} tracked).
          </CardDescription>
        </CardHeader>
        <CardContent className="space-y-3">
              ...
              >
                <div>
                  <div className="flex flex-wrap items-center gap-2">
                    ...
                    <Badge
                      className={siteUserIntentBadgeClass(siteUser.intent)}
                    >
                  ...
                  <p className="text-muted-foreground mt-1 text-xs">
                    {formatSiteUserTimestamp(
                      siteUser.updated_at || siteUser.created_at,
                    )}
                  </p>
                </div>
      ...
      >
        <DialogContent className="sm:max-w-lg">
          <DialogHeader>
            ...
            <DialogDescription>
              Update this student&apos;s display name or remove them from the
              class roster.
            </DialogDescription>
          </DialogHeader>
            ...
            <div className="space-y-4">
              ...
              <div className="flex flex-wrap items-center justify-between gap-3">
                <Button
                  ...
                  disabled={
                    !canManageRoster || removingStudentId === selectedStudent.id
                  }
                >
      ...
      <div className="flex items-start justify-between gap-3">
        <div>
          <p className="font-medium">
            {profile?.full_name || `User ${shortId(member.account_id)}`}
          </p>
          <p className="text-muted-foreground text-sm">
          ...
          <p className="text-muted-foreground mt-1 text-xs">
            {formatJoinedLabel(member.joined_at)}
          </p>
        </div>
...
function accessSummary(role: string) {
  if (role === 'owner')
    return 'Full GoClub control, including attendance and member management.';
  if (role === 'admin')
    return 'Can manage collaborators, students, and attendance workflows.';
  if (role === 'member')
    return 'Can contribute to club content and manage attendance.';
  if (role === 'viewer')
    return 'Read-only access to club content and attendance records.';
...
function upsertProjectInvitation(
  current: ProjectInvitation[],
  nextInvitation: ProjectInvitation,
) {
  const index = current.findIndex(
    (item) => item.invited_account_id === nextInvitation.invited_account_id,
  );
...
function upsertWebsiteAccount(
  current: WebsiteAccount[],
  nextAccount: WebsiteAccount,
) {
  const index = current.findIndex((item) => item.id === nextAccount.id);
  if (index < 0)
    return [...current, nextAccount].sort((a, b) =>
      a.name.localeCompare(b.name),
    );
